...
public class SupervisorPanel extends JPanel {
    ...
    private void loadAllSupervisors() {
        ...
        for (User user : supervisors) {
            if (user.getRole().equals(inventory.models.enums.UserRole.SUPERVISOR)) {
    ...
    private void addSupervisor() {
        ...
        if (username != null && password != null && controller.addSupervisor(username, password)) {
            loadAllSupervisors();
        } else {
            JOptionPane.showMessageDialog(this, "Failed to add supervisor.");
        }
    }
